...
const Producto = () => {
    ...
    const handleSearch = async (e) => {
        ...
        try {
            ...
            if (!Folio.some(a => a.id_producto === res.data.id_producto && a.id_status !== 1)) {
                alert('No hay ningun folio de este producto disponible para enviar a Almacen');
                // setErrorBusqueda(true);
                return false;
            }
    ...
    //         };
    //         await axios.post("http://localhost:85/backend/su_punto_final_para_insertar", datos);
    //       }
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            if (producto.length > 0) {
                let resConfirm = window.confirm("¿Desea enviar estos folios a Almacen?");
                if (resConfirm) {
                    producto.forEach(async element => {
                        if (element.num_producto) {
                            let query = `${window.backendHostUrl}/producto_envio_almacen/${element.id_producto}/${element.num_producto}`;
                    ...
                    window.alert('Folios enviado a Almacen Exitosamente');
                    navigate("/");
                }
            }
        } catch (err) {
            ...
            <div class="tableFixHead">
                <table id="tablaRegistros">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select name="num_producto" required onChange={(option) => funcSetFolio(index, option)}>
                                            ...
                                            {Folio.map((folio) => {
                                                if (folio.id_producto === prod.id_producto && folio.id_status !== 1)
                    ...
                    <tfoot>
                        <tr>
                            <td colSpan="5">
                                <button className="btn_new" onClick={handleUpdate}>Enviar a "Almacen"</button>
